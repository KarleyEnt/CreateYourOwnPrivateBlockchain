...
class BlockchainController {
  ...
  // putModifyBlock() {
  //   this.app.put("/modify/height/:height", async (req, res) => {
  //     if (req.params.height) {
  //       const height = parseInt(req.params.height);
  //       let block = await this.blockchain.getBlockByHeight(height);
  //       if (block) {
  //         const blockModification = await this.blockchain.modifyBlock(block);
  //         return res.status(200).send(JSON.stringify(block));
  //       }
  //     }
  //   });
  // }
  ...
  validateChain() {
    this.app.get("/validateChain", async (req, res) => {
      ...
      if (errorLog.length > 0) {
        return res.status(200).send('Chain has been altered');
      }
